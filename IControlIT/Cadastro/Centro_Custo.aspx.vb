...
Public Class Centro_Custo
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            Call Master.Titulo( _
                ...
                "Cadastro de Centro de Custo", _
                vTraduzir.Traduzir(Session("Conn_Banco"), Master.FindControl("ContentPlaceHolder1"), Page.Request.Path, Session("Id_Idioma"))))

            '-----voltar
            Call Master.Voltar("javascript:history.go(-1);", Nothing)
            Page.SetFocus(txtCentroCusto)
            Page.Form.DefaultButton = btPDF.UniqueID
            Call Master.Localizar("sp_Drop_Centro_Custo", Page.AppRelativeVirtualPath.ToString)

            If Not Request("ID") = Nothing Then
                vdataset = WS_Cadastro.Centro_Custo(Session("Conn_Banco"), Request("ID"), Nothing, Nothing, Nothing, "sp_SL_ID", True)
                txtIdentificacao.Text = vdataset.Tables(0).Rows(0).Item("Id_Centro_Custo")
                txtCentroCusto.Text = vdataset.Tables(0).Rows(0).Item("Cd_Centro_Custo")
                txtDescricao.Text = vdataset.Tables(0).Rows(0).Item("Nm_Centro_Custo")

                Dim fl_Desativado As Int16 = vdataset.Tables(0).Rows(0).Item("Fl_Desativado")

                '-----monta cadastro de hierarquia
                vdataset = WS_Cadastro.Relacionamento(Session("Conn_Banco"), "sp_Cadastro_Hierarquia", txtIdentificacao.Text, "CENTRO_CUSTO")
    ...
    Protected Sub btGrupo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btGrupo.Click
        '-----carrega consumidor
        WS_Cadastro.Credentials = System.Net.CredentialCache.DefaultCredentials
        oConfig.CarregaList(lstOrigem, WS_Cadastro.DropList(Session("Conn_Banco"), "sp_Drop_Filial", txtGrupo.Text))
    End Sub
    ...
    Protected Sub btSalvar_Click(sender As Object, e As EventArgs)
        WS_Cadastro.Credentials = System.Net.CredentialCache.DefaultCredentials
        txtIdentificacao.Text = WS_Cadastro.Centro_Custo(Session("Conn_Banco"),
                                                                oConfig.ValidaCampo(txtIdentificacao.Text),
                                                                oConfig.ValidaCampo(txtCentroCusto.Text),
        ...
        WS_Cadastro.Credentials = System.Net.CredentialCache.DefaultCredentials
        WS_Cadastro.Relacionamento(Session("Conn_Banco"), "sp_Rl_Hi_Filial_Centro_Custo_Insere", txtIdentificacao.Text, oConfig.AgrupaDados(lstDestino))

        '-----registro salvo ok
        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "alert", "Modal('#myModalRegistroSalvo');", True)
    End Sub

    Protected Sub btDesativar_Click(sender As Object, e As EventArgs)
        If Trim(txtIdentificacao.Text) = "" Then Exit Sub
        WS_Cadastro.Credentials = System.Net.CredentialCache.DefaultCredentials
        WS_Cadastro.Centro_Custo(Session("Conn_Banco"),
    ...
    Protected Sub btVoltar_Click(sender As Object, e As EventArgs)
        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Cadastro|Centro_Custo|btVoltar_Click", "javascript:history.go(-1);", True)
    End Sub
    ...
    Protected Sub btRestaurar_Click(sender As Object, e As EventArgs) Handles btRestaurar.Click
        ...
        WS_Cadastro.Credentials = System.Net.CredentialCache.DefaultCredentials
        WS_Cadastro.Lixeira(Session("Conn_Banco"), txtIdentificacao.Text, Session("Id_Usuario"), "Centro_Custo")
        pnlConfirmacao.Visible = False
        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "key", "enableScrolling();", True)
        Response.Redirect("Centro_Custo.aspx" & "?id=" & Request("ID"))
    End Sub
End Class
