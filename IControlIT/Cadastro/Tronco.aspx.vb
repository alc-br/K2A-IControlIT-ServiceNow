...
Public Class Tronco
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            Call Master.Localizar("sp_Drop_Tronco", Page.AppRelativeVirtualPath.ToString)

            oConfig.CarregaCombo(cboConglomerado, WS_Cadastro.DropList(Session("Conn_Banco"), "sp_Drop_Conglomerado", Nothing))
            oConfig.CarregaCombo(cboAtivoStatus, WS_Cadastro.DropList(Session("Conn_Banco"), "sp_Drop_Tronco_Grupo", Nothing))
            If Not Request("ID") = Nothing Then
                vdataset = WS_Cadastro.Tronco(Session("Conn_Banco"), Request("ID"), Nothing, Nothing, Nothing, Nothing, "sp_SL_ID", True)
                txtIdentificacao.Text = vdataset.Tables(0).Rows(0).Item("Id_Tronco")
                txtDescricao.Text = vdataset.Tables(0).Rows(0).Item("Nm_Tronco")
                ...
                cboAtivoStatus.SelectedValue = IIf(vdataset.Tables(0).Rows(0).Item("Id_Tronco_Grupo") = 0, "", vdataset.Tables(0).Rows(0).Item("Id_Tronco_Grupo"))
            End If
        End If
    End Sub
    ...
    Protected Sub btSalvar_Click(sender As Object, e As EventArgs)
        ...
        WS_Cadastro.Credentials = System.Net.CredentialCache.DefaultCredentials
        WS_Cadastro.Tronco(Session("Conn_Banco"),
